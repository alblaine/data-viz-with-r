...
Packages are programs that you import into R to help make tasks easier. The most popular R packages for working with data include the following: *dplyr, stringr, tidyr, and ggplot2*. 
...
## ggplot2: the (often) most important package to learn first
...
Essentially: graphs are like sentences you can construct, and they have a grammar. 

![](images/grammar-example.png)

## The Grammar of Graphics

According to the grammar of graphics, every chart should have at least one layer that contains the following:

* **data** & **aesthetic mapping** of the data to variables
* **stat** - a statistical transformation of the data -- sum, mean, count, fit line, etc. (optional)
* **geom** - geometric object (geom) -- point, line, bar, etc
* **position adjustment** - stacking, jitter, etc. (optional)
...
## Saving charts in ggplot2: ggsave()
...
## Advanced example with geom_text
In this example, we will label the total number of cars from each manufacturer in the mpg dataset and place those labels in a bar plot showing proportion of different classes for each manufacturer.
...
## Advanced exercise: Now create a similar bar plot using the diamonds dataset labeling the total number of diamonds for each type of clarity and showing the proportion of their cut types.
